...
# AGENTS.md + CHANGELOG.md carry the same path references as DOCS.md
# (AGENTS.md documents the state files/paths; CHANGELOG.md quotes the
# oauth discovery URL). Same universal renames, applied when present.
for doc in AGENTS.md CHANGELOG.md; do
    [[ -f "$doc" ]] && sed -i \
        -e 's|/api/mcp_proxy/oauth|/api/mcp_proxy_dev/oauth|g' \
        -e 's|/config/\.mcp_proxy_|/config/.mcp_proxy_dev_|g' \
        -e 's|/config/custom_components/mcp_proxy/|/config/custom_components/mcp_proxy_dev/|g' \
        -e 's|/opt/mcp_proxy|/opt/mcp_proxy_dev|g' \
        "$doc"
done
