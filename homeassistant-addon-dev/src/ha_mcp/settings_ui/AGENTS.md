...
- **Assets must sit beside `__init__.py`.** The loader resolves them relative to `__file__`. If you move them, update `Path(__file__).parent / ...`, plus `pyproject.toml` `[tool.setuptools.package-data]` and `MANIFEST.in` (all three list `settings_ui/settings.html`, `settings_ui/settings.js`, and `settings_ui/settings.css`). `tests/src/unit/test_resources.py` enforces the packaging entries.
