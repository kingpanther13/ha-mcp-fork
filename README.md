...
A `ha_manage_*` tool combines several operations, so it is reachable from every proxy: `ha_call_read_tool` runs only its read actions (the same per-call verdict Read Only Mode uses), while `ha_call_write_tool` and `ha_call_delete_tool` run the whole tool. Search results list each proxy the tool can be reached through. In Read Only Mode only `ha_call_read_tool` is registered.
