...
- Implemented the stable Supervisor datasource and real stable image inputs. Removed the initial manual Core 2026.9.1 bump at the user’s request; all five Core pins stay at 2026.9.0 so the later live Renovate dispatch can demonstrate the update end to end.
...
- At `5a92a443`, repository-mode validation and the actual Renovate HA/ordinary schedule, age, and limit fixtures passed. CI then exposed a separate image assumption: the slim image bundles Node but no npm. Move the expression-library installation to a setup-node runner step and mount its output read-only into the pinned engine container. Reply to and resolve the two Codex threads after the remaining behavioral fixtures pass. CodeRabbit automatically resolved its verified package-root finding. The live scanner is not deployed yet.
