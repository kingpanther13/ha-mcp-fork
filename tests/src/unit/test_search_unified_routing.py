...
async def test_unified_component_serves_listings_and_locations(
    ...
    calls = [c for c in ws.send_command.call_args_list if c.args[0] == "ha_mcp_tools/search"]
    assert len(calls) == 1
...
@pytest.mark.asyncio
async def test_explicit_config_pin_explains_skipped_entities(tmp_path: Any, monkeypatch: Any) -> None:
    _setup_visibility_disabled(tmp_path, monkeypatch)
    ws = make_ws(
        "ha_mcp_tools/search",
...
def test_unified_component_large_dashboard_window_is_serviceable() -> None:
    ...
    req = tools_search._ResolvedSearch(
        query="kitchen", query_text="kitchen", domain_filter=None,
        area_filter=None, state_filter=None, parsed_search_types=["automation", "dashboard"],
        parsed_fields=None, result_fields=None, limit=100, offset=600,
        exact_match=True, include_hidden=True, include_config=False,
        group_by_domain=False, per_domain_limit=None, config_time_budget=None,
        registry_eligible=False, body_eligible=True, body_skipped_by_intent_gate=False,
    )
    caps = SimpleNamespace(capabilities=frozenset({"search_unified"}), limits={"max_results": 500})
