...
async def test_component_inventory_pages_without_rest_states(scene_client, monkeypatch):
    ...
    from ha_mcp.tools.component_api import ComponentCaps, DEVICE_REGISTRY_CHILD_SEMANTICS

    caps = ComponentCaps(1, "2.1.0", frozenset({"search", DEVICE_REGISTRY_CHILD_SEMANTICS}), {"max_results": 1})
    monkeypatch.setattr(discovery, "get_component_caps", AsyncMock(return_value=caps))
    ws = MagicMock()
    ws.send_command = AsyncMock(side_effect=[
        {"result": {"entities": [{"entity_id": "scene.renamed", "friendly_name": "Movie"}], "entity_has_more": True}},
        {"result": {"entities": [{"entity_id": "scene.hue", "friendly_name": "Hue Relax"}], "entity_has_more": False}},
    ])
    ...
    assert all(call.kwargs["search_types"] == ["entity"] for call in ws.send_command.await_args_list)


async def test_component_inventory_failure_falls_back_to_rest(scene_client, monkeypatch):
    from ha_mcp.tools import scene_discovery as discovery
    from ha_mcp.tools.component_api import ComponentCaps, DEVICE_REGISTRY_CHILD_SEMANTICS

    caps = ComponentCaps(1, "2.1.0", frozenset({"search", DEVICE_REGISTRY_CHILD_SEMANTICS}), {})
    monkeypatch.setattr(discovery, "get_component_caps", AsyncMock(return_value=caps))
    monkeypatch.setattr(discovery, "get_websocket_client", AsyncMock(side_effect=OSError("offline")))
    result = await ConfigSceneTools(scene_client).ha_config_get_scene()
    assert result["total"] == 2
    scene_client.get_states.assert_awaited_once()


async def test_content_timeout_cancels_reads_and_reports_incomplete_total(scene_client, monkeypatch):
    from ha_mcp.tools import scene_discovery as discovery
    ...
    result = await ConfigSceneTools(scene_client).ha_config_get_scene(query="rainbow", search_in_config=True)
    assert result["partial"] is True
...
async def test_registry_component_read_preserves_storage_identity(scene_client, monkeypatch):
    from ha_mcp.tools import scene_discovery as discovery

    lookup = AsyncMock(return_value={"entities": scene_client.send_websocket_message.return_value["result"]})
